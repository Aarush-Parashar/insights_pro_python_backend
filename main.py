...
class PreprocessingConfig(BaseModel):
    # Frontend now sends empty config, so we mostly ignore this dict
    config: Dict[str, str] 
    target_variable: str
...
# REMOVED: @app.get("/predict/questions") 
# Redundant because frontend no longer asks these questions.


# 2. Predict - Step 2: The "AI Selection" Engine (Core Logic)
...
async def ai_model_selection(file_path: str, config: PreprocessingConfig, user: Any = Depends(get_current_user)):
    ...
    Uses hardcoded best-practice defaults since frontend sends empty config.
    """
    
    # A. Load Data
    df = get_dataframe_from_storage(file_path)
    
    # Cap data size for Render Free Tier
    if len(df) > 5000:
    ...
    # 1. Numeric Transformer (Defaults: Median & MinMaxScaler)
    numeric_transformer = Pipeline(steps=[
        ('imputer', SimpleImputer(strategy='median')), 
        ('scaler', MinMaxScaler())
    ])

    # 2. Categorical Transformer (Defaults: Most Frequent & Ordinal Encoding)
    # Using OrdinalEncoder handles unknown values better for tree-based models
    ...
    # Models Configuration (Optimized for Low RAM)
    if is_classification:
        models = {
